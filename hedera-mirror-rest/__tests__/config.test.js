...
describe('Validate redis config', () => {
  ...
  describe('Valid redis config with sentinels', () => {
    const testSpecs = [
      {
        name: 'Disabled with non-empty sentinel name',
        config: {
          enabled: false,
          sentinel: {
            name: 'test',
    ...
    testSpecs.forEach((spec) => {
      test(spec.name, async () => {
        expect(async () => await loadCustomConfig(customConfig(spec.config))).not.toThrow();
      });
    });

    testSpecs.forEach((spec) => {
      const config = _.omit(spec.config, 'sentinels');
      test(`${spec.name} - sentinels in ENV`, async () => {
        process.env['HEDERA_MIRROR_REST_REDIS_SENTINEL_SENTINELS'] = JSON.stringify(
          spec.config.sentinel.sentinels ?? []
        );
        expect(async () => await loadCustomConfig(customConfig(config))).not.toThrow();
      });
    });
  });

  describe('Invalid redis sentinel config', () => {
    const testSpecs = [
      {
      ...
      {
        name: 'Sentinel address missing host',
        sentinels: [{host: 'localhost', port: 26379}, {port: 26379}],
      },
      {
    ...
    testSpecs.forEach((spec) => {
      test(spec.name, async () => {
        ...
        await expect(loadCustomConfig(customConfig(redisConfig))).rejects.toThrow();
      });
    });
    ...
    testSpecs.forEach((spec) => {
      ...
      test(`${spec.name} - sentinels in ENV`, async () => {
        process.env['HEDERA_MIRROR_REST_REDIS_SENTINEL_SENTINELS'] = JSON.stringify(spec.sentinels);
        await expect(loadCustomConfig(customConfig(redisConfig))).rejects.toThrow();
      });
    });
