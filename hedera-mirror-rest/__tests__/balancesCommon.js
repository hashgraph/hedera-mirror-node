...
/**
 * Validate length of the balances response returned by the api
 *
 * @param {Object} resp response object returned by the rest api
 * @param {Number} len Expected length
 * @return {Boolean} Result of the check
 */
const validateLen = (resp, len) => {
  return resp.balances.length === len;
};

/**
 * Validate the range of timestamps in the balances response returned by the api
 *
 * @param {Object} resp response object returned by the rest api
 ...
 * @return {Boolean}  Result of the check
 */
const validateTsRange = (resp, low, high) => {
  const ret = resp.timestamp >= low && resp.timestamp <= high;

  if (!ret) {
    console.log(`validateTsRange check failed: ${resp.timestamp} is not between ${low} and  ${high}`);
  }
  return ret;
};

/**
 * Validate the range of account ids in the balances response returned by the api
 *
 * @param {Object} resp response object returned by the rest api
 ...
 * @return {Boolean}  Result of the check
 */
const validateAccNumRange = (resp, low, high) => {
  let ret = true;
  let offender = null;
  for (const balance of resp.balances) {
    const accNum = balance.account.split('.')[2];
...
/**
 * Validate the range of account balances in the balances response returned by the api
 *
 * @param {Object} resp response object returned by the rest api
 ...
 * @return {Boolean}  Result of the check
 */
const validateBalanceRange = (resp, low, high) => {
  let ret = true;
  let offender = null;
  for (const balance of resp.balances) {
    if (balance.balance < low || balance.balance > high) {
      offender = balance.balance;
      ret = false;
    }
  }
  if (!ret) {
...
/**
 * Validate the order of account IDs in the balances response returned by the api
 *
 * @param {Object} resp response object returned by the rest api
 * @param {String} order Expected order ('asc' or 'desc')
 * @return {Boolean}  Result of the check
 */
const validateOrder = (resp, order) => {
  let ret = true;
  let offenderAcc = null;
  let offenderVal = null;
  const direction = order === 'desc' ? -1 : 1;
  const toAccNum = (acc) => acc.split('.')[2];
  let val = toAccNum(resp.balances[0].account) - direction;
  for (const balance of resp.balances) {
    if (val * direction > toAccNum(balance.account) * direction) {
      offenderAcc = toAccNum(balance);
 ...
 * @param api
 */
const runTestSuite = (api, suiteName) => {
  describe(`{suitName} tests`, () => {
    // First, execute the single tests
    for (const [name, item] of Object.entries(singletests)) {
      test(`${suiteName} single test: ${name} - URL: ${item.urlparam}`, async () => {
    ...
    for (const combination of combinedtests) {
      ...
      test(`${suiteName} combination test: ${combtest.names} - URL: ${comburl}`, async () => {
