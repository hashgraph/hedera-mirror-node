...
class TransferListItemizer {
  ...
  itemize() {
    ...
    // Aggregate all transfers from the TransactionRecord (transaction result) into
    ...
    // Extract non_fee_transfers from unallocatedTransferAmounts
    ...
    // Extract node and network+service fees.
    ...
    // The unallocated map should be all zeroes. If not, add those amounts to the resultTransfers list.
    Object.keys(state.unallocatedTransferAmounts).forEach(accountId => {
      let amount = state.unallocatedTransferAmounts[accountId];
      if (!amount.isZero()) {
        state.extract(accountId, amount);
