...
| systemShardNum | `0` | The Hedera Hashgraph shard num supported by this system |
| addressBookFile | `"./config/0.0.102"` | The location of the address book file file |
| accountBalancesInsertBatchSize | `2000` | Performance tuning. Size of the batch DB insert operation while processing account balance files |
| accountBalancesFileBufferSize | `200000` | Performance tuning. Size of the file buffer used during account balance DB inserts |
| accountBalancesUseTransaction | `false` | Performance tuning. Is the entire set of of inserts for each account balance file surrounded by a DB transaction with rollback? |
