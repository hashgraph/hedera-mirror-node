 ...
 * account), a {@code contractID} key is <i>also</i> used to control permissions for an entity. The difference is that a
 * {@code contractID} key requires a particular contract to be executing in order to count as having "signed".
 *
 ...
 * current frame is {@code 0.0.C}, then this EVM transaction can use system contracts to, for example, transfer all the
 * hbar from account {@code 0.0.X}.
 */
