...
class OpcodeTracerCallsTest extends ContractCallTestSetup {
    ...
    @Captor
    private ArgumentCaptor<CallServiceParameters> paramsCaptor;

    @Captor
    private ArgumentCaptor<Long> gasCaptor;

    @Captor
    private ArgumentCaptor<TracerType> tracerTypeCaptor;

    @Captor
    private ArgumentCaptor<ContractCallContext> ctxCaptor;

    private final ResultCaptor<?> resultCaptor = new ResultCaptor<>(HederaEvmTransactionProcessingResult.class);
    ...
    private void verifyCaptors(final CallServiceParameters params) {
