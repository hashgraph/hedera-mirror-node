...
class ApprovePrecompileTest {
    ...
    void decodeApproveForNFTERC() {
        final var decodedInput = decodeTokenApprove(APPROVE_NFT_INPUT_ERC, TOKEN_ID, false, identity(), mirrorState);
    ...
    void decodeApproveForTokenERC() {
        given(mirrorState.typeOf(any())).willReturn(TokenType.FUNGIBLE_COMMON);
        final var decodedInput = decodeTokenApprove(APPROVE_TOKEN_INPUT_ERC, TOKEN_ID, true, identity(), mirrorState);
    ...
    void decodeApproveForNFTHAPI() {
        ...
        given(mirrorState.typeOf(any())).willReturn(TokenType.NON_FUNGIBLE_UNIQUE).willReturn(TokenType.FUNGIBLE_COMMON);
        final var decodedInput = decodeTokenApprove(APPROVE_NFT_INPUT_HAPI, null, false, identity, mirrorState);
        ...
        assertThrows(
                ...
                () -> decodeTokenApprove(APPROVE_NFT_INPUT_HAPI, null, false, identity, mirrorState));
    }

    @Test
    void decodeApproveForTokenAHPI() {
        UnaryOperator<byte[]> identity = identity();
        given(mirrorState.typeOf(any())).willReturn(TokenType.FUNGIBLE_COMMON).willReturn(TokenType.NON_FUNGIBLE_UNIQUE);

        final var decodedInput = decodeTokenApprove(APPROVE_TOKEN_INPUT_HAPI, null, true, identity, mirrorState);
        ...
        assertThrows(
                IllegalArgumentException.class,
                () -> decodeTokenApprove(APPROVE_TOKEN_INPUT_HAPI, null, true, identity, mirrorState));
    }
}
