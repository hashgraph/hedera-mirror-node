...
class MintLogicTest {
    ...
    void followsHappyPath() {
        ...
        verify(store).updateToken(token);
        verify(store).updateTokenRelationship(treasuryRel);
    }
    ...
    void followsUniqueHappyPath() {
        ...
        // when:
        subject.mint(token.getId(), 0, metadataList, Instant.now(), store);

        // then:
        verify(token).mint(any(OwnershipTracker.class), eq(treasuryRel), eq(metadataList), any(RichInstant.class));
        verify(store).updateToken(token);
        verify(store).updateTokenRelationship(treasuryRel);
