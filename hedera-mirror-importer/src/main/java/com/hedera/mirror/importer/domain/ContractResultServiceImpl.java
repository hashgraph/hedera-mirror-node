...
public class ContractResultServiceImpl implements ContractResultService {
    ...
    public void process(@NonNull RecordItem recordItem, Transaction transaction) {
        ...
        if (!isContractCreateOrCall(transactionBody)
                && !isValidContractFunctionResult(functionResult)) {
            // if transaction is neither a create/call and has no valid ContractFunctionResult then
            // skip
            return;
        }
        ...
        var transactionHandler =
                transactionHandlerFactory.get(TransactionType.of(transaction.getType()));

        // in pre-compile case transaction is not a contract type and entityId will be of a
        // different type
        var contractId =
                isContractCreateOrCall(transactionBody)
    ...
    private void processContractAction(
        ...
        switch (action.getCallerCase()) {
            case CALLING_CONTRACT -> contractAction.setCaller(
                    EntityId.of(action.getCallingContract()));
            case CALLING_ACCOUNT -> contractAction.setCaller(
                    EntityId.of(action.getCallingAccount()));
            default -> throw new InvalidDatasetException(
                    "Invalid caller for contract action: " + action.getCallerCase());
        }
        ...
        switch (action.getRecipientCase()) {
            ...
            case RECIPIENT_ACCOUNT -> contractAction.setRecipientAccount(
                    EntityId.of(action.getRecipientAccount()));
        ...
        switch (action.getResultDataCase()) {
            ...
            case REVERT_REASON -> contractAction.setResultData(
                    DomainUtils.toBytes(action.getRevertReason()));
            case OUTPUT -> contractAction.setResultData(DomainUtils.toBytes(action.getOutput()));
            default -> throw new InvalidDatasetException(
                    "Invalid result data for contract action: " + action.getResultDataCase());
        }
    ...
    private void processContractResult(
        ...
        List<Long> contractIds =
                getCreatedContractIds(functionResult, recordItem, contractEntityId);
        ...
        var transactionHash =
                Optional.ofNullable(recordItem.getEthereumTransaction())
        ...
        if (isValidContractFunctionResult(functionResult)) {
            if (!isContractCreateOrCall(recordItem.getTransactionBody())) {
                // amount, gasLimit and functionParameters were missing from record proto prior to
                // HAPI v0.25
                ...
                contractResult.setFunctionParameters(
                        DomainUtils.toBytes(functionResult.getFunctionParameters()));
            }

            contractResult.setBloom(DomainUtils.toBytes(functionResult.getBloom()));
            contractResult.setCallResult(
                    DomainUtils.toBytes(functionResult.getContractCallResult()));
    ...
    private void processContractLogs(
            ContractFunctionResult functionResult, ContractResult contractResult) {
        for (int index = 0; index < functionResult.getLogInfoCount(); ++index) {
            ...
            contractLog.setContractId(
                    lookup(contractResult.getContractId(), contractLoginfo.getContractID()));
    ...
    private void processContractStateChange(
        ...
        for (var storageChange : stateChange.getStorageChangesList()) {
            ...
            // If a value of zero is written the valueWritten will be present but the inner value
            // will be
    ...
    private List<Long> getCreatedContractIds(
        ...
        for (ContractID createdContractId : functionResult.getCreatedContractIDsList()) {
            ...
            if (!EntityId.isEmpty(contractId)) {
                ...
                // The parent contract ID can also sometimes appear in the created contract IDs
                // list, so exclude it
        ...
        for (var sidecarRecord : sidecarRecords) {
            ...
            if (sidecarRecord.hasStateChanges()) {
                ...
                for (var stateChange : stateChanges.getContractStateChangesList()) {
                    processContractStateChange(
                            consensusTimestamp, migration, payerAccountId, stateChange);
                }
            } else if (sidecarRecord.hasActions()) {
                var actions = sidecarRecord.getActions();
                for (int actionIndex = 0;
                        actionIndex < actions.getContractActionsCount();
                        actionIndex++) {
                    processContractAction(
                            actions.getContractActions(actionIndex),
                            consensusTimestamp,
                            actionIndex,
                            payerAccountId);
                }
    ...
    /**
     * Updates the contract entities in ContractCreateResult.CreatedContractIDs list. The method
     * should only be called for such contract entities in pre services 0.23 contract create
     * transactions. Since services 0.23, the child contract creation is externalized into its own
     * synthesized contract create transaction and should be processed by
    ...
    private void updateContractEntityOnCreate(Entity entity, RecordItem recordItem) {
        ...
        // No need to check initcode and other newer fields since they weren't available in older
        // HAPI versions
    ...
    /**
     * Persist contract entities in createdContractIDs if it's prior to HAPI 0.23.0. After that the
     * createdContractIDs list is also externalized as contract create child records so we only need
     * to persist the complete contract entity from the child record.
     *
    ...
    /**
     * This method works around a services issue that occurs when events emitted by a CREATE2
     * contract produce an invalid ContractID. The EVM generated logs contain the 20 byte CREATE2
     * EVM address and services does not properly convert this back to the 'shard.realm.num' format
     * that should always be present in the record. A similar issue occurs for state changes.
     *
     * <p>We work around this issue by converting the invalid 'shard.realm.num' format back to an
     * EVM address and look it up in the database. If the invalid contract ID is produced by a
     * CREATE2 constructor invocation, it won't be present in the database yet and our only recourse
     * is to fall back to using the root contract ID.
     *
     * <p>This issue never made it to mainnet, so this code should be deleted after testnet is
     * reset.
     *
    ...
    private EntityId lookup(EntityId rootContractId, ContractID contractId) {
        try {
            // We won't always get a negative or very large number to cause an
    ...
    private EntityId fallbackLookup(EntityId rootContractId, ContractID contractId) {
        ...
        log.warn(
                "Invalid ContractID {}.{}.{}. Attempting conversion to EVM address: {}",
