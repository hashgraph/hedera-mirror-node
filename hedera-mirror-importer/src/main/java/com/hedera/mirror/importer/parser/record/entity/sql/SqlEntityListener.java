...
public class SqlEntityListener implements EntityListener, RecordStreamFileListener {
    ...
    private void executeBatches() {
        ...
        try {
            ...
            nftPgCopy.copy(nfts.values(), connection);
    ...
    private Nft mergeNft(Nft cachedNft, Nft newNft) {
        // weigh options around account being nullable and order of events, mint, transfer, burn/wipe
        // set null account when newNft has no metadata. This differentiates between mint operation and an burn/wipe
        boolean setAccountId = EntityId.isEmpty(newNft.getAccountId()) ? newNft.getMetadata() == null : true;
        if (setAccountId) {
