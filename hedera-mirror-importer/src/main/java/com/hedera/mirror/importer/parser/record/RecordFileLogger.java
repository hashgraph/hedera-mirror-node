...
public class RecordFileLogger {
    ...
    private static PreparedStatement sqlInsertNonFeeTransfers;
    ...
    public static void storeRecord(Transaction transaction, TransactionRecord txRecord, byte[] rawBytes) throws Exception {
        ...
        if ((txRecord.hasTransferList()) && parserProperties.isPersistCryptoTransferAmounts()) {
            ...
            insertNonFeeTransfers(consensusNs, payerAccountId, body, txRecord);
        }
    ...
    private static boolean isApplicableToNonFeeTransfersTable(TransactionBody body, TransactionRecord transactionRecord) {
    ...
    /**
     * Additionally store rows in the non_fee_transactions table if:
     * <ul>
     *     <li>This is a successful CryptoCreate, ContractCreate, CryptoTransfer, or ContractCall transaction</li>
     *     <li>The TransactionRecord's transferlist does not appear itemized</li>
     * </ul>
     * This will allow the rest-api to create an itemized set of transfers that reflects non-fees (explicit transfers),
     * threshold records, node fee, and network+service fee (paid to treasury).
     *
     ...
     */
    private static void insertNonFeeTransfers(long consensusTimestamp, AccountID payerAccountId,
                                              TransactionBody body, TransactionRecord transactionRecord)
            throws SQLException {
        if (!isApplicableToNonFeeTransfersTable(body, transactionRecord)) {
            return;
        }

        for (var aa : nonFeeTransfersExtractor.extractNonFeeTransfers(payerAccountId, body, transactionRecord)) {
            addNonFeeTransfer(consensusTimestamp, aa.getAccountID().getRealmNum(), aa.getAccountID().getAccountNum(),
                    aa.getAmount());
        }
    }

    private static void addNonFeeTransfer(long consensusTimestamp, long realm, long accountNum, long amount)
            throws SQLException {
        if (0 != amount) {
            sqlInsertNonFeeTransfers.setLong(F_NONFEETRANSFER.CONSENSUS_TIMESTAMP.ordinal(), consensusTimestamp);
            sqlInsertNonFeeTransfers.setLong(F_NONFEETRANSFER.AMOUNT.ordinal(), amount);
            sqlInsertNonFeeTransfers.setLong(F_NONFEETRANSFER.REALM_NUM.ordinal(), realm);
    ...
    /**
     * For R3-style transferlists, which are itemized, the treasury should have multiple Transfers. Otherwise assume
     * aggregate. This method is only a good heuristic for successful transactions with non-fee transfers.
     *
     * @param transferList
     * @return
     */
