...
@Named
final class ScheduleDeleteTransformer extends AbstractBlockItemTransformer {
        ...
        for (var stateChanges : blockItem.stateChanges()) {
            for (var stateChange : stateChanges.getStateChangesList()) {
                if (stateChange.getStateId() == STATE_ID_SCHEDULES_BY_ID.getNumber() && stateChange.hasMapUpdate()) {
                    ...
                    if (value.hasScheduleValue()) {
                        var schedule = value.getScheduleValue();
                        if (schedule.getDeleted()) {
                            transactionRecordBuilder.getReceiptBuilder().setScheduleID(schedule.getScheduleId());
                            return;
                        }
                    }
                }
    ...
    public TransactionType getType() {
        return TransactionType.SCHEDULEDELETE;
    }
}
