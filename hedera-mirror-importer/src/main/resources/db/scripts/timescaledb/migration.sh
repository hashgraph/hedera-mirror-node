...
. scripts/timescaledb-migration/migration.config

if [[ -z $OLD_DB_HOST ]]; then
    echo "Old host name is not set. Please configure OLD_DB_HOST in migration.config file and rerun './timescaleDBMigration.sh'"
    exit 1
fi

if [[ -z $OLD_DB_NAME ]]; then
    echo "Old db name is not set. Please configure OLD_DB_NAME in migration.config file and rerun './timescaleDBMigration.sh'"
    exit 1
fi

if [[ -z $OLD_DB_PORT ]]; then
    echo "Old port is not set. Please configure OLD_DB_PORT in migration.config file and rerun './timescaleDBMigration.sh'"
    exit 1
fi

if [[ -z $OLD_DB_USER ]]; then
    echo "Old db primary user name is not set. Please configure OLD_DB_USER in migration.config file and rerun './timescaleDBMigration.sh'"
    exit 1
fi
...
PGPASSWORD=${NEW_PASSWORD} psql -h $NEW_DB_HOST -d $NEW_DB_NAME -p $NEW_DB_PORT -U $NEW_DB_USER -f scripts/timescaledb-migration/createHyperTables.sql

echo "5. Backing up tables from from Postgres($OLD_DB_HOST:$OLD_DB_PORT) to separate CSV's..."
PGPASSWORD=${OLD_PASSWORD} psql -h $OLD_DB_HOST -d $OLD_DB_NAME -p $OLD_DB_PORT -U $OLD_DB_USER -f scripts/timescaledb-migration/csvBackupTables.sql
...
PGPASSWORD=${NEW_PASSWORD} psql -h $NEW_DB_HOST -d $NEW_DB_NAME -p $NEW_DB_PORT -U $NEW_DB_USER -f scripts/timescaledb-migration/csvRestoreTables.sql

echo "7. Alter schema on TimescaleDB($NEW_DB_HOST:$NEW_DB_PORT) to support improved format..."
PGPASSWORD=${NEW_PASSWORD} psql -h $NEW_DB_HOST -d $NEW_DB_NAME -p $NEW_DB_PORT -U $NEW_DB_USER -f scripts/timescaledb-migration/alterSchema.sql
