...
class EventFileReaderV3Test {
    ...
    void readValidFileVersion2() {
        StreamFileData validFile = createEventFile(
                EventFileReaderV3.EVENT_STREAM_FILE_VERSION_2,
                EventFileReaderV3.EVENT_TYPE_PREV_HASH,
                PREVIOUS_HASH,
                CONTENT);
    ...
    void readValidFileVersion3() {
        StreamFileData validFile = createEventFile(
                EventFileReaderV3.EVENT_STREAM_FILE_VERSION_3,
                EventFileReaderV3.EVENT_TYPE_PREV_HASH,
                PREVIOUS_HASH,
                CONTENT);
        EventFile eventFile = eventFileReader.read(validFile);
    ...
    void readInvalidFileWithInvalidPrevHashMarker() {
        StreamFileData invalidFile =
                createEventFile(EventFileReaderV3.EVENT_STREAM_FILE_VERSION_3, (byte) 0x0, PREVIOUS_HASH, CONTENT);
        assertThrows(InvalidEventFileException.class, () -> eventFileReader.read(invalidFile));
    }

    @Test
    void readInvalidFileWithNoPrevHash() {
        StreamFileData invalidFile = createEventFile(
                EventFileReaderV3.EVENT_STREAM_FILE_VERSION_3, EventFileReaderV3.EVENT_TYPE_PREV_HASH, null, null);
        assertThrows(InvalidEventFileException.class, () -> eventFileReader.read(invalidFile));
    }
    ...
    private void verifyForSuccess(
            EventFile eventFile, StreamFileData inputFile, int expectedFileVersion, byte[] expectedPrevHash) {
        long consensusStart =
                DomainUtils.convertToNanosMax(inputFile.getStreamFilename().getInstant());
