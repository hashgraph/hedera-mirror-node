...
class ContractLogsConvertTopicsToBytesMigrationTest extends IntegrationTest {
    ...
    void verifyRootContractIdMigration() throws Exception {
        ...
        assertAll(
                () -> assertEquals(contractLogs.size(), 2),
                () -> assertEquals(contractLogs.get(0).consensusTimestamp, 1),
                () -> assertArrayEquals(contractLogs.get(0).getTopic0Bytes(), new byte[] {0}),
                () -> assertArrayEquals(contractLogs.get(0).getTopic1Bytes(), new byte[] {-86}),
                () -> assertArrayEquals(contractLogs.get(0).getTopic2Bytes(), new byte[] {-69}),
                () -> assertArrayEquals(contractLogs.get(0).getTopic3Bytes(), new byte[] {-52}),
                () -> assertEquals(Utility.bytesToHexString(contractLogs.get(0).getTopic0Bytes()), "00"),
                () -> assertEquals(Utility.bytesToHexString(contractLogs.get(0).getTopic1Bytes()), "aa"),
                () -> assertEquals(Utility.bytesToHexString(contractLogs.get(0).getTopic2Bytes()), "bb"),
                () -> assertEquals(Utility.bytesToHexString(contractLogs.get(0).getTopic3Bytes()), "cc"),
                () -> assertEquals(contractLogs.get(1).consensusTimestamp, 2),
