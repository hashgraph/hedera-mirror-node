...
public class NodeIdS3StreamProviderTest extends S3StreamFileProviderTest {
    ...
    protected FileCopier getFileCopier(Path fromPath, Path toPath) {
        // HIP-679: {network}/{shard}/{nodeID}/record/ - shard 0, node with account ID 0.0.3 is node ID 0.
        String network =
                properties.getMirrorProperties().getNetwork().toString().toLowerCase();
        return FileCopier.create(TestUtils.getResource(fromPath.toString()).toPath(), toPath, PathType.NODE_ID)
