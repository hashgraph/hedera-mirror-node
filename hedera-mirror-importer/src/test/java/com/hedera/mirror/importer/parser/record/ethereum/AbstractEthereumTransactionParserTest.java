...
abstract class AbstractEthereumTransactionParserTest {
    ...
    protected abstract EthereumTransactionBody getTransactionBody();

    protected abstract void validateEthereumTransaction(EthereumTransaction ethereumTransaction);

    @Test
    public void parse() {
        var ethereumTransaction = ethereumTransactionParser.parse(getTransactionBody());
        Assertions.assertThat(ethereumTransaction).isNotNull();

        assertThat(ethereumTransaction)
                .isNotNull()
                .satisfies(t -> assertThat(t.getConsensusTimestamp()).isGreaterThan(0))
