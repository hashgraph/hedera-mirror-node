...
public class RecordFileLoggerCryptoTest extends AbstractRecordFileLoggerTest {
	...
	void cryptoCreateTransactionWithBody() throws Exception {
		...
		assertAll(
            ...
            // Crypto transfer list
            ,() -> assertEquals(Utility.timeStampInNanos(record.getConsensusTimestamp()), dbCryptoTransfer.getConsensusTimestamp())
            ,() -> assertEquals(recordReceiptAccountId.getRealmNum(), dbCryptoTransfer.getAccountRealmNum())
            ,() -> assertEquals(recordReceiptAccountId.getAccountNum(), dbCryptoTransfer.getAccountNum())
        );
	...
	void cryptoUpdateFailedTransaction() throws Exception {
		...
		final com.hedera.mirror.domain.Transaction dbCreateTransaction = transactionRepository.findById(Utility.timeStampInNanos(createRecord.getConsensusTimestamp())).get();
