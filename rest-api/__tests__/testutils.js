...
const checkSql = function(parsedparams, condition) {
    for (const p of parsedparams) {
        if (p.field == condition.field &&
            p.operator == condition.operator &&
            p.value == condition.value) {
                return (true);
        }
    }
    console.log (`ERROR: Condition: ${condition.field} ${condition.operator} ${condition.value} not present in the generated SQL`);
    return (false);
}
...
const parseSqlQueryAndParams = function(sqlquery, sqlparams, orderprefix = '') {
    try {
        ...
        positionalparams.forEach( p => {
            ...
            if (matches.length === 4) { // original, field, operator, value
                parsedparams.push ({
                    field: matches[1],
                    operator: matches[2],
