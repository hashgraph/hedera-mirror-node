...
| transaction | type, consensus_timestamp                     | REST API | `/api/v1/transactions?type=:type&order=asc` | Used to retrieve transactions filtered by `type` and sorted by `consensus_timestamp` to facilitate faster by-type transaction requests |
