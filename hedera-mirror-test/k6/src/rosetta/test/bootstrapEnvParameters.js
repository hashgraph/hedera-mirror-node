...
const setupTestParameters = () => {
  ...
  const networkName = computeNetworkInfo(baseUrl).DEFAULT_NETWORK;
  const accountParameters = computeAccountParameters({baseApiUrl: `${baseUrl}${urlPrefix}`});

  const blockInfo = computeBlockFromNetwork(baseUrl, networkName);
  ...
  const transactionInfo = computeTransactionFromBlock(baseUrl, networkIdentifier, blockIdentifier);
  ...
  return {
    ...
    transactionIdentifier: {
      hash: transactionInfo.DEFAULT_TRANSACTION_HASH
    },
