...
Feature: Historical Feature
  ...
  Scenario Outline: Validate Historical Data
    ...
    Given I create "FUNGIBLE_KYC_UNFROZEN" token
    Given I create "NFT_HISTORICAL" token
...
#    Then I verify the response from non existing account
    Then I verify that historical data for "FUNGIBLE_KYC_UNFROZEN" is returned via getTokenInfo
    Then I verify that historical data for "NFT_HISTORICAL" is returned via getTokenInfo
    Then I verify that historical data for "FUNGIBLE_KYC_UNFROZEN" is returned via balanceOf
    Then I verify that historical data for "NFT_HISTORICAL" is returned via balanceOf
    Then I verify that historical data for "FUNGIBLE_KYC_UNFROZEN" is returned via balanceOf by direct call
    Then I verify that historical data for "NFT_HISTORICAL" is returned via balanceOf by direct call
    Then I verify historical data for "FUNGIBLE_KYC_UNFROZEN" is returned for allowance
    Then I verify historical data for "NFT_HISTORICAL" is returned for getApproved
    Then I verify historical data for "FUNGIBLE_KYC_UNFROZEN" is returned for ERC allowance
    Then I verify historical data for "NFT_HISTORICAL" is returned for ERC getApproved
    Then I verify historical data for "FUNGIBLE_KYC_UNFROZEN" is returned for allowance by direct call
