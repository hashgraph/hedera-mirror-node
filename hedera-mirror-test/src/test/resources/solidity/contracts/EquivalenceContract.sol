...
contract EquivalenceContract {
    function makeCallWithoutAmount(address _to, bytes memory _data) public returns (bool success, bytes memory returnData){
        (success, returnData) = _to.call(_data);
    }

    function makeCallWithAmount(address _to, bytes memory _data) public payable returns (bool success, bytes memory returnData){
        (success, returnData) = _to.call{value: msg.value}(_data);
    }

    function makeStaticCall(address _to, bytes memory _data) public returns (bool success, bytes memory returnData){
        (success, returnData) = _to.staticcall(_data);
    }

    function makeDelegateCall(address _to, bytes memory _data) public returns (bool success, bytes memory returnData){
    ...
    function getCodeHash(address _addr) public view returns (bytes32) {
        bytes32 codehash;
        assembly {
    ...
    function copyCode(address _addr) public view returns (bytes memory) {
        uint256 size;
        assembly {
            size := extcodesize(_addr)
        }
        ...
        assembly {
            extcodecopy(_addr, add(code, 32), 0, size)
        }
        return code;
    }
}
