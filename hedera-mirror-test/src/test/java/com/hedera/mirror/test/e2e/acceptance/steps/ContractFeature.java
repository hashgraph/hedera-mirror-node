...
public class ContractFeature {
    ...
    private static final int MAX_FILE_SIZE = 64000; // 64kb
    private final ObjectMapper mapper = new ObjectMapper();
    @Value("classpath:solidity/artifacts/contracts/MirrorNodeShards.sol/MirrorNode.json")
...
//        Path addressBookPath = ResourceUtils
//                .getFile("classpath:solidity/artifacts/contracts/MirrorNodeShards.sol/MirrorNode.json").toPath();
//        ResourceUtils.getFile(mirrorNodeContract.toUri());
        ...
        persistContractBytes(compiledSolidityArtifact.getBytecode());
    ...
    public void updateContract() {
        networkTransactionResponse = fileClient.createFile(SMART_CONTRACT_BYTECODE);
        assertNotNull(networkTransactionResponse.getTransactionId());
        assertNotNull(networkTransactionResponse.getReceipt());
    ...
    private void verifyContractInfo(ContractInfo contractInfo) {
        ...
        assertThat(contractInfo.balance.toTinybars()).isGreaterThan(0);
    ...
    public void verifyNetworkContractDeleteResponse() {
        var contractInfo = contractClient.getContractInfo(contractId);
        assertThat(contractInfo.isDeleted).isTrue();
    }
    ...
    private void persistContractBytes(String contractContents) {
        ...
        while (byteIndex <= contractBytes.length) {
            ...
            String fileContents = String.valueOf(Arrays.copyOfRange(contractBytes, byteIndex, stopIndex));
